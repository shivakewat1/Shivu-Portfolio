...
const contactInfo = [
  { icon: FiMail, label: 'Email', value: 'shiva@example.com', href: 'mailto:shiva@example.com' },
    ...
    <section id="contact" className="py-28 relative" ref={ref}>
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <p className="mt-4 text-base max-w-md" style={{ color: '#4a4a4a' }}>
          ...
          >
            {contactInfo.map(({ icon: Icon, label, value, href }) => (
              <a
                key={label}
                href={href}
                className="flex items-center gap-4 glass glass-hover rounded-2xl p-4 transition-all group"
              >
                <div
                  className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
                  style={{ background: 'rgba(201,181,156,0.1)' }}
                >
                  <Icon size={16} style={{ color: '#C9B59C' }} />
                </div>
                <div>
                  <div className="text-xs mb-0.5" style={{ color: '#3a3a3a' }}>{label}</div>
                  <div className="text-sm font-medium" style={{ color: '#8a7a6a' }}>{value}</div>
                </div>
              </a>
            ))}
